...
Type.Function = class extends Type {
  ...
  getName() {
    if (this.name === null) {
      const params = this.paramTypes.length !== 0
            ? `<${this.paramTypes.map(type => type.getName()).join(', ')}>`
            : ''

      this.name = `Fn<${this.returnType.getName()}${params}>`
    }
...
Type.Union = class extends Type {
  ...
  equals(other) {
    ...
    return this.optionTypes.some(type => type.equals(other));
  }
}
...
Type.GenericFunction = class extends Type {
  constructor({name = null, genericTypesStr, params, body, returnType, env}) {
    ...
    this.genericTypes = genericTypesStr;
    this.params = params;
    this.returnType = returnType;
    this.body = body;
    this.env = env;
